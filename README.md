...
- `prompts/`: 6개 모듈 프롬프트 + 일괄 오케스트레이터 + X/Grok 수동 아이디어 수집용 engineered prompt template
- `examples/humanoid-household/`: 샘플 브리프와 코퍼스
- `docs/architecture.md`: 구조 설명
- `docs/manual-x-grok-idea-sourcing.md`: 수동 X/Grok 아이디어 수집 절차와 이번 실행 기록
- `ideas/manual-x-grok/YYYY-MM-DD/`: 수동 수집 아이디어 txt, status ledger, raw captures
- `outputs/storyboards/YYYY-MM-DD/`: 수동으로 선별한 storyboard 결과물
- `tools/manual/`: 루틴 자동화가 아닌 agent 수동 조작 보조 스크립트
